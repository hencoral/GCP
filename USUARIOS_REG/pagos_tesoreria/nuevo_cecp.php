...
?>
	...
	<head>
		...
		<SCRIPT language="javascript">
			function MostrarOcultarDEDU(objetoVisualizar) {
				if (document.all[objetoVisualizar].style.display == '') {
					document.all[objetoVisualizar].style.display = 'none';
				} else {
					document.all[objetoVisualizar].style.display = '';
				}
			}
		...
		<SCRIPT language="javascript">
			function MostrarOcultar(objetoVisualizar) {
				...
				if (document.all['naturales'].style.display == '') {
					document.all['naturales'].style.display = '';
					document.a.ter_nat.disabled = false;
					document.all['juridicos'].style.display = 'none';
					document.getElementById('ter_jur').value = '';
		...
		<SCRIPT language="javascript">
			function MostrarOcultar2(objetoVisualizar) {
				...
				if (document.all['juridicos'].style.display == '') {
					document.all['naturales'].style.display = 'none';
					document.getElementById('ter_nat').value = '';
					document.a.ter_nat.disabled = true;
					document.all['juridicos'].style.display = '';
		...
		<script>
			...
			function rubroDuplicado(id) {
				var cont = 0;
				for (i = 1; i <= 20; i++) {
					var valor = document.getElementById('valcourse_' + i).value;
					if (valor) {
						for (j = 1; j <= 20; j++) {
							var existe = document.getElementById('valcourse_' + j).value;
							if (valor == existe) {
								cont++;
								if (cont > 1) {
									alert("La cuenta ya ha sido seleccionada");
									document.getElementById('valcourse_' + j).value = '';
								}
							}
						}
						var cont = 0;
			...
			function VerificaSaldo(id) {
				...
				var rubro = document.getElementById('valcourse_' + id).value;
				if (rubro == '') {
				...
				//var aux=document.getElementById('cuenta_cxp1').value;
				//alert (aux);
			}
			...
			function validarForm() {
				...
				for (i = 1; i <= filas; i++) {
					var cuenta = document.getElementById('valcourse_' + i).value;
					if (cuenta == '') {
		...
		<script>
			...
			function ejecucion_pto() {
				document.getElementById('muestra_informe').style.display = "";
				var fila = parseFloat(document.getElementById('contis2').innerHTML);
				var cods = new Array();
				for (i = 1; i <= fila; i++) {
					cods[i] = document.getElementById('valcourse_' + i).value;
				}
			...
			function muestra_saldo(id) {
				var input_cuenta = "valcourse_" + id.substring(9);
		...
		<script>
			...
			function llenartabla() {
				...
				agregar();
				var s = parseFloat(document.all.tablaf.rows.length);
		...
		<script>
			...
			function sumacredito() {
				...
				//alert (acum);
				document.getElementById('tot_cre_a').value = Math.round(sumatotal * 100) / 100;
			}
			...
			function sumadebito() {
				...
				document.getElementById('tot_deb_a').value = Math.round(sumatotal * 100) / 100;
			}
			...
			function cta_cecp(n) //  n2 => impar, n3 => par
			{
				...
				var cecp = document.getElementById('valcourse_' + n).value; //alert(cecp);
				var req = new XMLHttpRequest();
				if (req) {
					req.onreadystatechange = function() {
						if (req.readyState == 4 && (req.status == 200 || req.status == 304)) {
							...
							if (document.getElementById('iva').value == '') {
								...
								//var iva=1+(parseInt(document.getElementById('iva').value)/100);	
	...
	<body onload="ceros();">
		...
		$aut = isset($_GET["siauto"]) ? $_GET["siauto"] : '';
		$autiva = isset($_GET["riva"]) ? $_GET["riva"] : '';
		$autestun = isset($_GET["autestun"]) ? $_GET["autestun"] : '';
		$autestdo = isset($_GET["autestdo"]) ? $_GET["autestdo"] : '';
		$autesttr = isset($_GET["autesttr"]) ? $_GET["autesttr"] : '';
		$autestcu = isset($_GET["autestcu"]) ? $_GET["autestcu"] : '';
		$autestci = isset($_GET["autestci"]) ? $_GET["autestci"] : '';
		?>
		...
		<form name="a" method="post" id="commentForm" action="p_cecp.php" />
		<table width="800" border="0" align="center">
			...
			<tr>
				<td colspan="3">
								...
								<div style="padding-left:15px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="left">
										...
										if (empty($_POST['fecha_cecpp'])) {
											...
											$fecha_cecp = $_POST['fecha_cecpp']; //printf("%s",$fecha_cecp); echo "entra else";
										}
										?>
										<input name="fecha_cecpp" type="text" class="required Estilo12" id="fecha_cecpp" value="<?php printf($fecha_cecp); ?>" size="12" />
									...
									<div align="center" class="Estilo12">
										<div align="center">
											...
											if ($ultimo2 == '') {
												...
												// una consulta que recorra todos lo campos
												$re5 = $cx->query("select distinct id_manu_cecp FROM cecp");
												while ($rw5 = $re5->fetch_row()) {
                    ...
                    Arial, Helvetica, sans-serif; font-size: 12px; font-weight: normal; border: #333333 
                     3px solid; background-color: #FAFAFA; color: #000000; display:none;">
							...
							<td colspan="3">
								<table border="0" align="left">
									<tr>
										...
										$chk =  isset($_POST['chkretefte']) ? $_POST['chkretefte'] : '';
										if (isset($_POST['Submit']) or isset($_POST['Submit2']) or isset($_POST['Submit3']) or isset($_POST['Submit32']) or $chk == "SIAUT" or $chk == "") {
												...
												<div align="left">
													<select name="ter_nat" class="Estilo4" id="ter_nat" style="width: 350px;" <?php print $enaternat; ?>>
														...
														while ($r = $rs->fetch_assoc()) {
															if ($r['id'] == $ter_nat or $r['id'] == '$ternatidaut' or $r['id'] == '$ternatidman') {
											...
											<div style="padding-left:5px; padding-top:0px; padding-right:5px; padding-bottom:0px;">
												<div align="left">
													<select name="ter_jur" class="Estilo4" id="ter_jur" style="width: 350px;" <?php print $enaterjur; ?>>
														...
														$rs = $db->query($strSQL);
														while ($r = $rs->fetch_assoc()) {
															if ($r['id'] == isset($_POST['ter_jur']) ? $_POST['ter_jur'] : '' or $r['id'] == '$terjuridaut' or $r['id'] == '$terjuridman') {
							...
							<td colspan="3" bgcolor="#FFFFFF">
								<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="left" class="Estilo4">
										<input name="concepto_pago" type="text" class="required Estilo4" id="concepto_pago" onkeyup="a.concepto_pago.value=a.concepto_pago.value.toUpperCase();" style="width: 520px;" value="<?php $conce = isset($_POST['concepto_pago']) ? $_POST['concepto_pago'] : '';
																																																								printf("%s", $conce); ?>" />
									</div>
								</div>
							</td>
						...
						for ($k = 1; $k <= 20; $k++) {
							$cuenta_cxp = isset($_POST['cuenta_cxp' . $k]) ? $_POST['cuenta_cxp' . $k] : '';
			  ...
			  <input type='text' name='course_$k' class='Estilo4' id='course_$k' size='95' />
                                <input type='hidden' name='cuenta_cxp$k' id='valcourse_$k' />
			  
			  </div>
			  </div>
			  </td> 
			  <td align='center'> <input type='text'  name='valor_pto$k' style='text-align:right' id='valor_pto$k' value='$valor_pto' onblur='VerificaSaldo($k);'  onkeypress='return validar(event)' ondblclick='muestra_saldo(id)'/> </td>
			  </tr>
			";
						...
						<tr>
							...
							<td>
								<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<?php $vr_obli_para_pago_mas_iva = isset($_POST['vr_obli_para_pago_mas_iva']) ? $_POST['vr_obli_para_pago_mas_iva'] : 0; ?>
											<input name="vr_obli_para_pago_mas_iva" type="text" class="required Estilo12" id="vr_obli_para_pago_mas_iva" style="text-align:right" onkeypress="return validar(event)" value="<?php printf("%.0f", $vr_obli_para_pago_mas_iva); ?>" onchange="valvrob()" readonly />
										</div>
									</div>
								</div>
							</td>
					...
					<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
						<div align="center" class="Estilo12">
							<div align="center">
								...
								?>
								<input name="vr_obli_para_pago_sin_iva" type="text" class="required Estilo12" id="vr_obli_para_pago_sin_iva" style="text-align:right" onkeypress="return validar(event)" value="<?php printf("%.0f", $vr_obli_para_pago_sin_iva); ?>" />
							</div>
						</div>
					</div>
				</td>
					...
					<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
						<div align="center" class="Estilo12">
							<div align="center">
								...
								?>
								<input name="iva_vr_obli_pago" type="text" class="required Estilo12" id="iva_vr_obli_pago" style="text-align:right" onkeypress="return validar(event)" value="<?php printf("%.0f", $iva_vr_obli_pago); ?>" />
							</div>
						</div>
					</div>
				</td>
		...
		<table width="800" border="0" id="deduciones" align="center" style="display:''">
			<tr>
				<td colspan="4">
					...
					<table width="800" border="1" align="center" class="bordepunteado1" id="dedu" style="display:''">
						<tr>
							<td colspan="4" bgcolor="#DCE9E5">
								<div style="padding-left:5px; padding-top:10px; padding-right:5px; padding-bottom:10px;">
						...
						<tr>
							...
							<td>
								<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="salud" type="text" class="Estilo12" id="salud" style="text-align:right" onkeypress="return validar(event)" value="<?php $salud = isset($_POST['salud']) ? $_POST['salud'] : 0;
																																											printf("%.0f", $salud);	 ?>" />
											<?php
											if ($autom[0] == 'SI')
								...
								<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="center" class="Estilo12">
										<div align="center">
											...
											<input name="pension" id="pension" type="text" class="Estilo12" style="text-align:right" onkeypress="return validar(event)" value="<?php $pension = isset($_POST['pension']) ? $_POST['pension'] : 0;
																																												printf("%.0f", $pension);	 ?>" />
											<?php
											if ($autom[1] == 'SI')
						...
						<tr>
							...
							<td>
								<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="libranza" type="text" class="Estilo12" id="libranza" style="text-align:right" onkeypress="return validar(event)" value="<?php $libranza = isset($_POST['libranza']) ? $_POST['libranza'] : 0;
																																													printf("%.0f", $libranza);	 ?>" />
											<?php
											if ($autom[2] == 'SI')
							...
							<td>
								<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="f_solidaridad" type="text" class="Estilo12" id="f_solidaridad" style="text-align:right" onkeypress="return validar(event)" value="<?php $f_solidaridad = isset($_POST['f_solidaridad']) ? $_POST['f_solidaridad'] : 0;
																																															printf("%.0f", $f_solidaridad);	 ?>" />
											<?php
											if ($autom[3] == 'SI')
						...
						<tr>
							...
							<td width="200">
								<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="f_empleados" type="text" class="Estilo12" id="f_empleados" style="text-align:right" onkeypress="return validar(event)" value="<?php $f_empleados = isset($_POST['f_empleados']) ? $_POST['f_empleados'] : 0;
																																														printf("%.0f", $f_empleados);	 ?>" />
											<?php
											if ($autom[4] == 'SI')
							...
							<td width="200">
								<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="sindicato" type="text" class="Estilo12" id="sindicato" style="text-align:right" onkeypress="return validar(event)" value="<?php $sindicato = isset($_POST['sindicato']) ? $_POST['sindicato'] : 0;
																																													printf("%.0f", $sindicato);	 ?>" />
											<?php
											if ($autom[5] == 'SI')
						...
						<tr>
							...
							<td>
								<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="embargo" type="text" class="Estilo12" id="embargo" style="text-align:right" onkeypress="return validar(event)" value="<?php $embargo = isset($_POST['embargo']) ? $_POST['embargo'] : 0;
																																												printf("%.0f", $embargo);	 ?>" />
											<?php
											if ($autom[6] == 'SI')
							...
							<td>
								<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="cruce" type="text" class="Estilo12" id="cruce" style="text-align:right" onkeypress="return validar(event)" value="<?php $cruce = isset($_POST['cruce']) ? $_POST['cruce'] : 0;
																																											printf("%.0f", $cruce);	 ?>" />
											<?php
											if ($autom[7] == 'SI')
						...
						<tr>
							...
							<td>
								<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="otros" type="text" class="Estilo12" id="otros" style="text-align:right" onkeypress="return validar(event)" value="<?php $otros = isset($_POST['otros']) ? $_POST['otros'] : 0;
																																											printf("%.0f", $otros);	 ?>" />
											<?php
											if ($autom[8] == 'SI')
								...
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center">
										<select name="retefuente" class="Estilo12" id="retefuente" style="width: 300px;" onchange="retefuentef(value);">
											<option value=""></option>
											<?php
											$retefuente = isset($_POST['retefuente']) ? $_POST['retefuente'] : '';
											include('../config.php');
								...
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center" class="Estilo12">
										<div align="center">
											...
											<input name="vr_retefuente" type="text" class="Estilo12" id="vr_retefuente" style="text-align:right" onchange="ceros();" onkeypress="return validar(event)" value="<?php $vr_retefuente = isset($_POST['vr_retefuente']) ? $_POST['vr_retefuente'] : 0;
																																																				printf("%s", $vr_retefuente); ?>" />
										</div>
									</div>
								</div>
							</td>
								...
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center">
										<select name="reteiva" class="Estilo12" id="reteiva" style="width: 300px;" onchange="ret_iva(value)">
											<option value=""></option>
											<?php
											$reteiva = isset($_POST['reteiva']) ? $_POST['reteiva'] : '';
											include('../config.php');
									...
									<div align="center" class="Estilo12">
										<div align="center">
											...
											<!--              <input name="vr_reteiva" type="text" class="Estilo12" id="vr_reteiva" style="text-align:right" onkeypress="return validar(event)" value="<?php //printf("%.0f",$vr_reteiva); 
																																																		?>" />-->
											<input name="vr_reteiva" type="text" class="Estilo12" id="vr_reteiva" style="text-align:right" onchange="ceros();" onkeypress="return validar(event)" value="<?php $vr_reteiva = isset($_POST['vr_reteiva']) ? $_POST['vr_reteiva'] : 0;
																																																			printf("%s", $vr_reteiva); ?>" />
										</div>
									</div>
								</div>
						...
						<tr>
							<td>
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center"><span class="Estilo41">
											<input name="reteica" type="text" class="Estilo41" id="reteica" onkeyup="a.reteica.value=a.reteica.value.toUpperCase();" size="55" value="RETEICA" />
										</span></div>
								</div>
							</td>
							<td>
							...
							<td>
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="tarifa_reteica" type="text" class="Estilo12" id="tarifa_reteica" style="text-align:right" onkeyup="otro();" onkeypress="return validar(event)" value="0" />
										</div>
									</div>
								</div>
							</td>
							<td>
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="vr_reteica" type="text" class="Estilo12" id="vr_reteica" style="text-align:right" onchange="ceros();" onkeypress="return validar(event)" value="0" />
										</div>
									</div>
								</div>
							</td>
						...
						<tr>
								...
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center"><span class="Estilo41">
											<select name="estampilla1" class="Estilo12" id="estampilla1" style="width: 300px;" onchange="estamp1(value,id);">
												<option value=""></option>
												<?php
												$estampilla2 = isset($_POST['estampilla1']) ? $_POST['estampilla1'] : '';
												include('../config.php');
							...
							<td bgcolor="#F5F5F5">
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="vr_estampilla1" type="text" class="Estilo12" id="vr_estampilla1" onchange="ceros();" style="text-align:right" onkeypress="return validar(event)" value="0" />
										</div>
									</div>
								</div>
							</td>
						...
						<tr>
								...
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center"><span class="Estilo41">
											<select name="estampilla2" class="Estilo12" id="estampilla2" style="width: 300px;" onchange="estamp1(value,id);">
												<option value=""></option>
												<?php
												$estampilla2 = isset($_POST['estampilla2']) ? $_POST['estampilla2'] : '';
												include('../config.php');
												$query = "SELECT * FROM estampillas";
												$link = new mysqli($server, $dbuser, $dbpass, $database);
												$result = $link->query($query);
							...
							<td bgcolor="#FFFFFF">
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="vr_estampilla2" type="text" class="Estilo12" id="vr_estampilla2" onchange="ceros();" style="text-align:right" onkeypress="return validar(event)" value="0" />
										</div>
									</div>
								</div>
							</td>
						...
						<tr>
								...
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center"><span class="Estilo20">
											<select name="estampilla3" class="Estilo12" id="estampilla3" style="width: 300px;" onchange="estamp1(value,id);">
												<option value=""></option>
												<?php
												$estampilla3 = isset($_POST['estampilla3']) ? $_POST['estampilla3'] : '';
												include('../config.php');
												$query = "SELECT * FROM estampillas";
												$link = new mysqli($server, $dbuser, $dbpass, $database);
												$result = $link->query($query);
							...
							<td bgcolor="#F5F5F5">
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="vr_estampilla3" type="text" class="Estilo12" id="vr_estampilla3" onchange="ceros();" style="text-align:right" onkeypress="return validar(event)" value="0" />
										</div>
									</div>
								</div>
							</td>
						...
						<tr>
								...
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center"><span class="Estilo20">
											<select name="estampilla4" class="Estilo12" id="estampilla4" style="width: 300px;" onchange="estamp1(value,id);">
												<option value=""></option>
												<?php
												$estampilla4 = isset($_POST['estampilla4']) ? $_POST['estampilla4'] : '';
												include('../config.php');
												$query = "SELECT * FROM estampillas";
												$link = new mysqli($server, $dbuser, $dbpass, $database);
												$result = $link->query($query);
							...
							<td bgcolor="#FFFFFF">
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="vr_estampilla4" type="text" class="Estilo12" id="vr_estampilla4" onchange="ceros();" style="text-align:right" onkeypress="return validar(event)" value="0" />
										</div>
									</div>
								</div>
							</td>
						...
						<tr>
								...
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center"><span class="Estilo20">
											<select name="estampilla5" class="Estilo12" id="estampilla5" style="width: 300px;" onchange="estamp1(value,id);">
												<option value=""></option>
												<?php
												$estampilla5 = isset($_POST['estampilla5']) ? $_POST['estampilla5'] : '';
												include('../config.php');
												$query = "SELECT * FROM estampillas";
												$link = new mysqli($server, $dbuser, $dbpass, $database);
												$result = $link->query($query);
							...
							<td bgcolor="#F5F5F5">
								<div style="padding-left:5px; padding-top:2px; padding-right:5px; padding-bottom:2px;">
									<div align="center" class="Estilo12">
										<div align="center">
											<input name="vr_estampilla5" type="text" class="Estilo12" id="vr_estampilla5" onchange="ceros();" style="text-align:right" onkeypress="return validar(event)" value="0" />
										</div>
									</div>
								</div>
							</td>
								...
								<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
									<div align="center" class="Estilo41">
										<div align="center"><b>FORMA DE PAGO </b> <span class="Estilo12">
												...
												<?php $forma_pago = isset($_POST['forma_pago']) ? $_POST['forma_pago'] : '';
					...
					<div style="padding-left:5px; padding-top:10px; padding-right:5px; padding-bottom:10px;">
						<div align="center" class="Estilo4">
							<p><strong>MOVIMIENTO CONTABLE
									<input type="hidden" name='contador' value='<?php //print $contador1; 
																				?>' id="contador">
									<input type="hidden" name='contini' value='<?php //print $numpost; 
																				?>' id="contini">
								</strong><strong><br />
								</strong>
							...
							<p>
								<strong><img src="images/mas.png" alt="" title="Agregar Fila" width="20" height="20" border="0" style='cursor: pointer' ; onclick='agregar();'>
									&nbsp;&nbsp;&nbsp;&nbsp;
									<span id='contis' class='Estilo4'><?php //printf("%s", $contador1); 
																		?></span>
									&nbsp;&nbsp;&nbsp;&nbsp;
