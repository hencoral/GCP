...
?>
<form name="pagol" id="pagol" action="pagos_lote_proc" method="post">
...
for ($i=1;$i<=$filas;$i++)
{
	$dato[$i] = $_POST['camp_'.$i];
	if ($dato[$i] !='')
	{
		$k++;
		// consulta al obcg para obtener datos
		$sql ="select * from obcg where id_auto_obcg = '$dato[$i]'";
		...
		echo "<tr>
					...
					<td><input name='cult_$k' id='cult_$k' value='$procultura'  size='8' style='text-align:right' onblur='valores(id,value);' /></td>
					...
					<td bgcolor='#CC3'><input name='tpagar_$k' id='tpagar_$k' value=''  size='10' style='text-align:right;background-color:#CC3;border:0' /></td>
					<td><input name='deb_$k' id='deb_$k' value='$rw1[pgcp2]'  size='10' style='text-align:right'  /></td>
					<td><input name='valcre_$k' id='valcre_$k' value='11100507'  size='12'/>
					</td>
					<td><input name='cheq_$k' id='cheq_$k' value=''  size='6'  /></td>
			 </tr>";	
	}
}
