...
$id=$_GET["id"];
$i=0;
$actos = array("","CONSTITUCION GARANTIAS","ACTA DE INICIO","ADICION O PRORROGA","MODIFICACION","ACLARACION","SUSPENCION","INFORME INTERVENTORIA","LIQUIDACION");
for ($i=0;$i<9;$i++)
{
	if ($proced ==$actos[$i])
 	{
...
$vigencia = $anno[0];
// Datos del contrato
	$res2 = mysql_db_query($database,"select * from contrataciones2 where num_contrato ='$num_cont'",$cx);
	$row2 = mysql_fetch_array($res2);
	$clas_contrato = $row2["clas_contrato"]; 

// Datos del registro presupuestal 
	$res = mysql_db_query($database,"select * from crpp where id_auto_crpp ='$row2[id_auto_crpp]'",$cx);
...
$num_contrato =$vigencia."-".$clas_contrato.$row2["num_contrato"]; 
...
// Datos del acto de postcontratacion
	$res4 = mysql_db_query($database,"select * from postcontratacion where id ='$id'",$cx);
	$row4 = mysql_fetch_array($res4);
	$fecha_post = $row4["fecha_acto"];
	$fec_larga = strftime("%d d�as del mes de %B de %Y",strtotime("$fecha_post"));
	$cump = $row4["tipo_acta_interventoria"];
	if ($cump =='PARCIAL')
	{
		$cumplimiento = "ha venido cumpliendo";
	}
	if ($cump =='FINAL')
	{
		$cumplimiento = "ha cumplido";
	}	
	if ($cump =='OTRO')
	{
		$cumplimiento = $row4["descripcion"];
	}
