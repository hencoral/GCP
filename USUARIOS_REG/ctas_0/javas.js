function masitem(){
  ...
  if(document.getElementById('contador').value > 50){document.getElementById('contador').value='50';}
...
function rasca(val){
 veapues();
 var ide='',co1='',co2='',co3='', resulta='',pgcp='';
 var i = 0;
 for(i=3;i<=val;i++){
   ide = 'fil' + eval(i);
   xDisplay(ide,'block');
 }
 for(i=(parseInt(val)+1);i<=50;i++){
   co1 = 'vr_deb_'+eval(i);
   co2 = 'vr_cre_'+eval(i);
   co3='cheque_'+eval(i);
   resulta = 'resulta'+eval(i);
   pgcp = 'pgcp'+eval(i);
  ...
  if(valor != ''){
    ...
    { if (ajax.readyState==4) {
       ...
       if(res == 0){
        ...
        else{document.getElementById(resulta).innerHTML = ajax.responseText;}
    }
   }
...
function lookupg(inputString,ide) {
        var sugges = '#sugges' + eval(ide);
        var autoSug = '#autoSug' + eval(ide);
        if(inputString.length == 0) {
            $(sugges).hide();
        } else {
            $.post("rpc_gas.php", {queryString: ""+inputString+"",qid: ""+ide+""}, function(data){
                if(data.length >0) {
                    $(sugges).show();
                    $(autoSug).html(data);
                }
...
function lookupcxp(inputString,ide) {
        var sugges = '#sugges' + eval(ide);
        var autoSug = '#autoSug' + eval(ide);
        if(inputString.length == 0) {
            $(sugges).hide();
        } else {
            $.post("rpc_cxp.php", {queryString: ""+inputString+"",qid: ""+ide+""}, function(data){
                if(data.length >0) {
                    $(sugges).show();
                    $(autoSug).html(data);
                }
...
function fill(obj,ide) {
      ...
      var resulta = 'resulta' + eval(ide);
       ...
       for (i = 0; i < valor.length; i++) {
        ...
        document.getElementById(resulta).innerHTML = maso[1];
    }
...
function vertodas1(obj,ide){
  ...
  if(roco == comp){
   ...
   obj.rock = 'no'+ide;
   var ajax=objetoAjax();
   ajax.open("POST", "listodocesp.php");
    ajax.onreadystatechange=function()
    { if (ajax.readyState==4) {
        document.getElementById('cesp').innerHTML = ajax.responseText;
        xDisplay('espere','none');
        xDisplay('cesp','block');
    }
   }
...
function vertodas2(obj,ide){
  ...
  if(roco == comp){
   ...
   obj.rock = 'no'+ide;
   var ajax=objetoAjax();
   ajax.open("POST", "listodoncsp.php");
    ajax.onreadystatechange=function()
    { if (ajax.readyState==4) {
        document.getElementById('ncsp').innerHTML = ajax.responseText;
        xDisplay('espere','none');
        xDisplay('ncsp','block');
    }
   }
...
function vertodas3(obj,ide){
  ...
  if(roco == comp){
   ...
   obj.rock = 'no'+ide;
   var ajax=objetoAjax();
   ajax.open("POST", "listodondsp.php");
    ajax.onreadystatechange=function()
    { if (ajax.readyState==4) {
        document.getElementById('ndsp').innerHTML = ajax.responseText;
        xDisplay('espere','none');
        xDisplay('ndsp','block');
    }
   }
...
function vertodas4(obj,ide){
  ...
  if(roco == comp){
   ...
   obj.rock = 'no'+ide;
   var ajax=objetoAjax();
   ajax.open("POST", "listodotfin.php");
    ajax.onreadystatechange=function()
    { if (ajax.readyState==4) {
        document.getElementById('tfin').innerHTML = ajax.responseText;
        xDisplay('espere','none');
        xDisplay('tfin','block');
    }
   }
...
function vertodas5(obj,ide){
  var comp = 'si'+ide;
  var roco = obj.rock;
  if(roco == comp){
   xDisplay('caja10','none');
   xDisplay('espere','block');
   obj.rock = 'no'+ide;
   var ajax=objetoAjax();
   ajax.open("POST", "listodocoba.php");
    ajax.onreadystatechange=function()
    { if (ajax.readyState==4) {
        document.getElementById('coba').innerHTML = ajax.responseText;
        xDisplay('espere','none');
        xDisplay('coba','block');
    }
   }
