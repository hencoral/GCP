  ...
  $id_emp=$rowxx["id_emp"];
    ...
    if(isset($_POST['queryString']))
    {
        ...
        $id = $_POST['qid'];
            ...
            $query = mysql_query("Select * from ctas0_gas where cod_pptal_gas_apr like '".$queryString."%' and id_emp ='$id_emp';",$conexion);
                if($query) {
                        while($row2 = mysql_fetch_array($query))
                        {$r1=$row2["cod_pptal_gas_apr"];
                         $r2=$row2["nom_pptal_gas_apr"];
