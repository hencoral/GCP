...
$sq = "SELECT id_auto_ceva,id_manu_ceva,id_auto_crpp,id_auto_cobp,ccnit, vr_retefuente,vr_reteiva, total_pagado,id_auto_ceva, retefuente, iva,des_ceva FROM ceva";
...
while($rw = mysql_fetch_array($re)) 
{
	...
	$sq17= "select cuenta from lib_aux where id_auto = '$rw[id_auto_ceva]' and debito >0";
	$re17 = mysql_db_query($database, $sq17, $cx);
	while($rw17 = mysql_fetch_array($re17)) 
	{
		$cta .= $rw17['cuenta'] ."  "; 
	}
	...
	$retefuente2=$tarifa * $base;
		...
		// es pago acumulado
				...
				while($rw1 = mysql_fetch_array($re1))
				{
					...
					$mcipio =$rw1["mpio2"]; 
					$pais =$rw1["pais2"];
					$sq5="INSERT INTO dian_exogena (tipo,codigo_1001,codigo_1002,tipo_documento,documento,dv,razon_social,direccion,depto,mcipio,pais,valor,base,retencion,cuenta,nom_cuenta,concepto,id_ceva,cta) values('retefuente','$codigo_1001','$codigo_1002','$tipo_documento','$documento','$dv2','$razon_social','$direccion','$depto','$mcipio','$pais','$valor_pagado','$base','$retefuente2','$cuenta','$nom_rubro','$ceva . $des_ceva','$id_auto_ceva','$cta')";
					mysql_select_db ($database, $cx);
					mysql_query($sq5, $cx); 
					} 		
		...
		// no es pago acumulado	
			if (!empty($ter_nat))
			{
				...
				$re4 = mysql_db_query($database, $sq4, $cx);
					...
					$mcipio =$rw4["mpio"]; 
					$pais =$rw4["pais"];
					$sq6="INSERT INTO dian_exogena (tipo,codigo_1001,codigo_1002,tipo_documento,documento,dv,apellido_1, apellido_2,nombre_1,otros_nombres,direccion,depto,mcipio,pais,valor,base,retencion,cuenta,nom_cuenta,concepto,id_ceva,cta) values('retefuente','$codigo_1001','$codigo_1002','$tipo_documento','$documento','$dv','$pri_ape','$seg_ape','$pri_nom','$seg_nom','$direccion','$depto','$mcipio','$pais','$valor_pagado','$base','$retefuente2','$cuenta','$nom_rubro','$ceva . $des_ceva','$id_auto_ceva','$cta')";
					mysql_select_db ($database, $cx);
					mysql_query($sq6, $cx);
				} 		
...
// d ***** Consulto la base para llenar la tabla con los comprabantes de egreso vigencia atual CEVA
$sq = "SELECT id_manu_ceva,id_auto_crpp,id_auto_cobp,ccnit, vr_retefuente,vr_reteiva, total_pagado, reteiva,iva FROM ceva";
$re = mysql_db_query($database, $sq, $cx);
while($rw = mysql_fetch_array($re)) 
{
...
// *******************************************************************************************************************    RETENCIONES CECP 
$sq = "SELECT cn,concepto_pago,cuenta_cxp,vr_obli_para_pago_mas_iva,concepto_pago,retefuente,vr_retefuente,iva,id_auto_cecp,id_manu_cecp FROM cecp";
$re = mysql_db_query($database, $sq, $cx);
while($rw = mysql_fetch_array($re)) 
{
	...
	$sq17= "select cuenta from lib_aux where id_auto = '$rw[id_auto_cecp]' and debito >0";
	$re17 = mysql_db_query($database, $sq17, $cx);
	while($rw17 = mysql_fetch_array($re17)) 
	{
		$cta .= $rw17['cuenta'] ."  "; 
		...
		// ********* CONSULTO TERCEROS NATURALES	
			...
			if ($nat >0)
			{
					while($rw4 = mysql_fetch_array($re4))
					{
						...
						$mcipio =$rw4["mpio"]; 
						$pais =$rw4["pais"];
						$sq6="INSERT INTO dian_exogena (tipo,codigo_1001,codigo_1002,tipo_documento,documento,dv,apellido_1, apellido_2,nombre_1,otros_nombres,direccion,depto,mcipio,pais,valor,base,retencion,cuenta,nom_cuenta,concepto,id_ceva,cta) values('retefuente','$codigo_1001','$codigo_1002','$tipo_documento','$documento','$dv','$pri_ape','$seg_ape','$pri_nom','$seg_nom','$direccion','$depto','$mcipio','$pais','$valor_pagado','$base','$retefuente2','$cuenta','$nom_rubro','$des_cobp','$id_manu_cecp','$cta')";
						mysql_select_db ($database, $cx);
						mysql_query($sq6, $cx);
					} 		
...
$sq = "SELECT cn,concepto_pago,cuenta_cxp,vr_obli_para_pago_mas_iva,concepto_pago,retefuente,vr_reteiva,iva,reteiva,id_manu_cecp FROM cecp";
$re = mysql_db_query($database, $sq, $cx);
while($rw = mysql_fetch_array($re)) 
{
	$id_manu_cecp =$rw['id_manu_cecp'];
...
while($rw = mysql_fetch_array($re))
{
	$sq51 ="select sum(credito) as valorc from lib_aux where id_auto = '$rw[id_auto_ncon]' and cuenta like '2436%' and cuenta not like '243627%' and cuenta not like '243625%'";
	$re51 = mysql_db_query($database, $sq51, $cx);
	$rw51 = mysql_fetch_array($re51);
	$reten = $rw51['valorc'];
	$sq52 ="select sum(credito) as valoriva from lib_aux where id_auto = '$rw[id_auto_ncon]' and cuenta like '243625%'";
	$re52 = mysql_db_query($database, $sq52, $cx);
	$rw52 = mysql_fetch_array($re52);
	$reteiva = $rw52['valoriva'];

	$sq5="INSERT INTO dian_exogena (tipo,tipo_documento,documento,razon_social,valor,base,retencion,concepto,cuenta,id_ceva,reteiva) values('retefuente','13','$rw[ccnit]','$rw[tercero]','$rw[tot_deb]','0','$reten','$rw[des_ncon]','$rw[id_manu_ncon]','$rw[id_manu_ncon]','$reteiva')";
	mysql_select_db ($database, $cx);
    mysql_query($sq5, $cx); 
	$reten =0;
	$reteiva =0;
