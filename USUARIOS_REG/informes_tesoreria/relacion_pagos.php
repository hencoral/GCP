...
<td  bgcolor='#DCE9E5' align='center'>No. De CRPP</td>
<td  bgcolor='#DCE9E5' align='center'>No. De CONTRATO</td>
<td  bgcolor='#DCE9E5' align='center'>Total Obligacion</td>
<td  bgcolor='#DCE9E5' align='center'>Saldo por pagar</td>
<td  bgcolor='#DCE9E5' align='center'>Cod banco</td>
<td  bgcolor='#DCE9E5' align='center'>Tipo cta</td>
<td  bgcolor='#DCE9E5' align='center'>No cuenta</td>
</tr>
");
		...
		while ($rw = $re->fetch_assoc()) {
			...
			$sq7 = "select id_manu_cdpp,vr_digitado,n_contrato from crpp where id_auto_crpp ='$rw[id_auto_crpp]'";
			$re7 = $cx->query($sq7);
			$rw7 = $re7->fetch_assoc();
			...
			// consulta bancos y cuentas
			$sq8 = "select * from cuenta_bco where ccnit ='$rw[ccnit]'";
			//echo $sq8;
			$re8 = $cx->query($sq8) or die('Error A: ' . mysqli_error($cx));
			$rw8 = $re8->fetch_assoc();
			if ($rubros == 1) {
				$rw1 = $re1->fetch_assoc();
				...
				<td align='center' bgcolor='#CCCCCC'>%s</td>
				<td align='center' bgcolor='#CCCCCC'>%s</td>
				</tr>", $rw["fecha_ceva"], $rw1["cuenta"], ucfirst($rw5["nom_rubro"]), $rw["id_manu_ceva"], $rw["tercero"], $rw["ccnit"], ucfirst(preg_replace("[,;]", "", $rw["des_cobp"])), $valor, $seg_soc, $reten, $otros_des, $neto, $banco, $cunta_ban, $cheque, $rw1['id_manu_cdpp'], $rw1['id_manu_crpp'], $rw7['n_contrato'], $rw7['vr_digitado'], $saldo, $rw8['cod_banco'], $rw8['tipo_cta'], $rw8['numero_cta']);
				$banco = '';
				$cunta_ban = '';
				$cheque = '';
			} else {
				...
				while ($rw1 = $re1->fetch_assoc()) {
					...
					if ($rw1["pag"] > 0) {
						...
						printf("
								...
								<td align='center' bgcolor='#CCCCCC'>%s</td>
								<td align='center' bgcolor='#CCCCCC'>%s</td>
								</tr>", $rw["fecha_ceva"], $rw1["cuenta"], ucfirst($rw6["nom_rubro"]), $rw["id_manu_ceva"], $rw["tercero"], $rw["ccnit"], ucfirst(preg_replace("[,;]", "", $rw["des_ceva"])), $rw1["pag"], $seg_soc2, $reten2, $otros_des2, $neto2, $banco, $cunta_ban, $cheque, $rw1['id_manu_cdpp'], $rw1['id_manu_crpp'], $rw7['n_contrato'], $rw7['vr_digitado'], $saldo, $rw8['cod_banco'], $rw8['tipo_cta'], $rw8['numero_cta']);
						$banco = '';
						$cunta_ban = '';
						$cheque = '';
					}
