...
while($rw = $re->fetch_assoc())
{
		...
		if ($rubros == 1)
		{
		 ...
		 if ($clase =='')
				...
				</tr>",$rw["fecha_ceva"],$rw1["cuenta"],ucfirst($rw5["nom_rubro"]),$rw["id_manu_ceva"],$rw["tercero"],$rw["ccnit"],ucfirst(ereg_replace("[,;]", "",$rw["des_cobp"])),$valor,$seg_soc,$reten,$otros_des,$neto,$banco,$cunta_ban,$cheque,$rw['id_manu_crpp']);
		}else{
				...
				while($rw1 = $re1->fetch_assoc())
					{
						...
						if ($rw1["pag"] >0)
						{
							...
							printf("
								...
								<td align='center'>%s</td>
								<td align='center' bgcolor='#999999'>%s</td>
								</tr>",$rw["fecha_ceva"],$rw1["cuenta"],ucfirst($rw5["nom_rubro"]),$rw["id_manu_ceva"],$rw["tercero"],$rw["ccnit"],ucfirst(ereg_replace("[,;]", "",$rw["des_cobp"])),$rw1["pag"],$seg_soc2,$reten2,$otros_des2,$neto2,$banco,$cunta_ban,$cheque,$rw['id_manu_crpp']);
							}	
					}
