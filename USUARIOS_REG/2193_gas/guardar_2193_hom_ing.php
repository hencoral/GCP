...
//**** verifico si ya existe
	...
	$sql = "select * from 2193_gas_ok where cod_pptal='$cuenta'";
	$result = mysql_db_query($database, $sql, $cx) or die(mysql_error());
	if (mysql_num_rows($result) == 0)
	{
	
			$connectionxx = new mysqli($server, $dbuser, $dbpass, $database) or die ("Fallo en la Conexion a la Base de Datos");
			$sqlxx = "select * from 2193_gas where cod = '$cod_concepto_2193_ing'";
			...
			$sql = "INSERT INTO 2193_gas_ok ( cod , tipo, trimestre, concepto, cod_pptal, nom_rubro, definitivo)
	...
	else
	{
			...
			$sql = "update 2193_gas_ok set cod='$cod_concepto_2193_ing' , tipo='$tipo', trimestre='$trimestre', concepto='$concepto' where cod_pptal = '$cuenta' ";
