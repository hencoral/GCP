...
$re = mysql_db_query($database, $sq, $cx);
		while($rw = mysql_fetch_array($re))
		{
			...
			$banco = mysql_db_query($database,"select cod_sia from pgcp where cod_pptal ='$cuenta'",$cx);
			...
			$pagado = $rw["total_pagado"] * $parcial;
				...
				</tr>", $rwc["cod_sia"].$rw2["cuenta"], ucfirst(ereg_replace("[,;]", "",$rwc["nom_rubro"])), $rw2["valor"], $rw["fecha_cecp"],$per, $rw["id_manu_cecp"], $rw["nt"], $rw["cn"], ereg_replace("[,;]", "",$rw["concepto_pago"]),$val_comprobante, $seg_soc, $reten, $otros_desc, $pagado, $cod_banco, $cuenta , $chequen); 
			}
		}
printf("</table></center>");
}
