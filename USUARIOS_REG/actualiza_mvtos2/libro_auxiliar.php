...
mysql_query($sq0, $cx) or die(mysql_error());	 
...
mysql_select_db ($base, $cx);

		if(mysql_query ($tabla6,$cx)) 
		{
		echo "";
		}
		else
				...
				{	 
					...
					while($rrw2 = mysql_fetch_array($rr2)) 
					{
					  $nom_rubro2=$rrw2["nom_rubro"];
						...
						$rr23 = mysql_db_query($database, $sql52, $cx);
		...
		for($i=1;$i<16;$i++)
			{
				...
				{	 
					...
					$rr3 = mysql_db_query($database, $ss3, $cx);
...
$re = mysql_db_query($database, $sq, $cx);
$campo = mysql_num_fields($re);
$veces =16;
if ($campo > 500) $veces = 161;
while($rw = mysql_fetch_array($re)) 
{
		for($i=1;$i<$veces;$i++)
			{
