...
$cx = new mysqli($server, $dbuser, $dbpass, $database) or die ("Fallo en la Conexion a la Base de Datos");
		...
		$sql2 = "select max(id_manu_rcgt) from recaudo_rica1 where fecha_recaudo='$fecha'";
		...
		if ($concec)
		{
			...
			do {
					$con = $concec + $i;
					$con2 ="RCGT".$con;
					$sq2 =mysql_db_query($database,"select * from recaudo_rica1 where id_manu_rcgt ='$con2'",$cx);
					$fil = mysql_num_rows($sq2);
					$conant = $con-1;
					$conant2 = "RCGT".$conant;
					$sq3 =mysql_db_query($database,"select * from recaudo_rica1 where id_manu_rcgt ='$conant2'",$cx);
		...
		// si la fecha no tiene registros
			...
			do {
					...
					$sql4 = "select max(id_manu_rcgt) from recaudo_rica1 where fecha_recaudo='$fecha_b'";
