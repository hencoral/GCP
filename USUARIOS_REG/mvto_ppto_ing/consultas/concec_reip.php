...
if ($concec) {
	...
	do {
		$con = $concec + $i;
		$con2 = "REIP" . $con;
		$sq2 = $cx->query("select * from reip_ing where id_manu_reip ='$con2'");
		$fil = $sq2->num_rows;
		$conant = $con - 1;
		$conant2 = "REIP" . $conant;
		$sq3 = $cx->query("select * from reip_ing where id_manu_reip ='$conant2'");
		$row3 = $sq3->fetch_assoc();
		$fecha2 = isset($row3["fecha_reip"]) ? $row3["fecha_reip"] : '';
		if ($fil > 0) {
			$i++;
			$j = 0;
		} else {
