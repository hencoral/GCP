...
$anadir5="TRUNCATE TABLE `car_ppto_ing` ";
...
if(!mysql_select_db($database,$db))
	...
	// Calcular nivel de las cuentas
	  $n_cod = strlen($data[0]);
	  	if ($n_cod ==1) $nivel=1;
		if ($n_cod ==2) $nivel=2;
		if ($n_cod >2) $nivel = ceil(($n_cod/2)+1);
	...
	// Calcular la cuenta padre
		...
		if ($n_cod >1) 
		{
		$sql ="select * from car_ppto_ing where cod_pptal = '$padre'";
	...
	if($data[2]=='M') {$afectado='1.00';}else{$afectado='0.00';}
			   	   ...
			   	   $import="INSERT INTO car_ppto_ing ( 
					ano,
					id_emp,
					cod_pptal,
					padre,
