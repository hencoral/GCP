...
<td bgcolor='#DCE9E5' align='center'><b>Aprobado2</b></td>
...
while($rw = $re->fetch_assoc())
{
	...
	$sq1 = "SELECT SUM(ppto_aprob) as inicial from car_ppto_ing where  tip_dato ='D' and cod_pptal like '$cod_pptal%'";
	$re1 = mysql_db_query($database, $sq1, $cx);
	$rw1 = mysql_fetch_array($re1);
	// Consulto valores ADICION para cada cuenta
	$sq2 = "SELECT SUM(valor_adi) as adicion from adi_ppto_ing where    cod_pptal like '$cod_pptal%'";
	$re2= mysql_db_query($database, $sq2, $cx);
	$rw2 = $re2->fetch_assoc();
	// Consulto valor REDUCCION para cada cuenta
	$sq3 = "SELECT SUM(valor_adi) as reduccion from red_ppto_ing where  cod_pptal like '$cod_pptal%'";
	...
	$sq15 = "SELECT SUM(octubre) as octubre from pac_ingresos where cod_pptal like '$cod_pptal%'";
	$re15 = mysql_db_query($database, $sq15, $cx);
	$rw15 = mysql_fetch_array($re15);
	if (!$rw15["octubre"]) $rw15["octubre"] =0;
	// Consulto pagos NOVIEMBRE
	$sq16 = "SELECT SUM(noviembre) as noviembre from pac_ingresos where cod_pptal like '$cod_pptal%'";
	...
	$sq17 = "SELECT SUM(diciembre) as diciembre from pac_ingresos where cod_pptal like '$cod_pptal%'";
