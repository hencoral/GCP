...
if (!isset($_SESSION["login"])) {
	...
	$cod_vigencia = isset($_POST['vigencia']) ? $_POST['vigencia'] : null;
	$cod_ser_deuda = isset($_POST['ser_deuda']) ? $_POST['ser_deuda'] : null;
	$ent_recip = $_POST['ent_recip'];


	$cx = new mysqli($server, $dbuser, $dbpass, $database);

	$sSQL = "UPDATE car_ppto_gas Set cod_fut='$cod_fut' , fuentes_recursos='$fuentes_recursos' , unidad_ejecutora='$unidad_ejecutora' , situacion='$situacion' ,  cod_cgr='$cod_cgr' , cod_rec='$cod_rec' , oer='$oer' , cda='$cda' , vigencia_gasto='$vigencia_gasto' , finalidad_gasto='$finalidad_gasto' , uni_ejec_cgr='$uni_ejec_cgr', cod_vigencia='$cod_vigencia', cod_ser_deuda='$cod_ser_deuda', ent_recip='$ent_recip'  Where cod_pptal = '$cod_pptal' and id_emp ='$id_emp'";
	$cx->query($sSQL);
