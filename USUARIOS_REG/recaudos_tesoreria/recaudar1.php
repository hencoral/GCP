	...
	<head>
		...
		<script>
			function validar(e) {
				...
				if (tecla == 8 || tecla == 46 || tecla == 44) return true; //Tecla de retroceso (para poder borrar) 
				patron = /\d/; //ver nota 
		...
		<script>
			...
			function consecutivo2() {
				...
				var pos_url2 = '../recibo_caja/consultas/concec_rgct.php';
		...
		<script type="text/javascript">
			...
			function agregar() {
				...
				if (fila < 159) {
					...
					td = tr.insertCell();
					td.innerHTML = "<div style='padding-left:5px; padding-top:3px; padding-right:5px; padding-bottom:3px;'><span class='Estilo4'> <input type='text' size='20' style='text-align:left' id='pgcp" + contLin + "' name='pgcp" + contLin + "' value='' onkeyup='lookup(this.value," + contLin + ");' onkeydown='displaycode(event,id);' > </span></div> <div class='suggestionsBox' id='sugges" + contLin + "' style='display: none; position:absolute; left: 130px;'><img src='images/upArrow.png' style='position: relative; top: -10px; left: 0px;' title='PGCP'> <div class='suggestionList' id='autoSug" + contLin + "' align=left> &nbsp;  </div> </div>";

					td = tr.insertCell();
					td.innerHTML = "<input type='text' size='50' style='text-align:left' name='des" + contLin + "' id='des" + contLin + "' value='' readonly >";

					td = tr.insertCell();
					td.innerHTML = "<input type='text' size='20' style='text-align:right' name='vr_deb_" + contLin + "' id='vr_deb_" + contLin + "' value=0   onKeyUp='suma_tab();' onkeypress='return validar(event)' onblur='formato(id,value)' onfocus='siespgcp(" + contLin + ");' onkeydown='displaycode(event,id);' >";

					td = tr.insertCell();
					td.innerHTML = "<input type='text' size='20' style='text-align:right' name='vr_cre_" + contLin + "' id='vr_cre_" + contLin + "' value=0  onKeyUp='suma_tab();' onkeypress='return validar(event)' onblur='formato(id,value)' onkeydown='displaycode(event,id);' >";
					td = tr.insertCell();
			...
			function generar_movimiento() {
				...
				if (vacio != 1) {
					...
					for (j = 1; j <= filas; j++) {
						...
						var cuenta = document.getElementById("valcourse_" + j).value;
						...
						if (req1) {
							req1.onreadystatechange = function() {
								if (req1.readyState == 4) {
									var dato = req1.responseText;
									var elem = dato.split(',');
									// verifica si tiene movimeinto debito
									if (elem[0] != '') {
										k++;
										agregar();
										document.getElementById("pgcp" + k).value = elem[0];
										document.getElementById("des" + k).value = elem[1];
										document.getElementById("vr_deb_" + k).value = document.getElementById("valor_" + j).value;
										suma += parseFloat(document.getElementById("valor_" + j).value.replace(/\,/g, ''));
										ctrl = 1;
									}
									k++;
									agregar();
									document.getElementById("pgcp" + k).value = elem[2];
									document.getElementById("des" + k).value = elem[3];
									document.getElementById("vr_cre_" + k).value = document.getElementById("valor_" + j).value;
									suma += parseFloat(document.getElementById("valor_" + j).value.replace(/\,/g, ''));
								}
							}
					...
					// controla si entra bancos
					if (ctrl == 0) {
						agregar();
						document.getElementById("pgcp" + j).value = '1110';
						document.getElementById("vr_deb_" + j).value = formatea(suma);
					}
					suma_tab();
				}
			}
			...
			function tipo_cuenta(id) {
				...
				var pos_url2 = '../recibo_caja/consultas/tipo_cuenta.php';
				var req1 = new XMLHttpRequest();
				if (req1) {
					req1.onreadystatechange = function() {
						if (req1.readyState == 4) {
				...
				<tr>
					<td colspan="3">
						<div style="padding-left:10px; padding-top:30px; padding-right:10px; padding-bottom:10px;">
							<div align="center" class="Estilo4"><strong>RECIBO DE CAJA GENERAL </strong>
								<?php
								...
								$num_filas = 1;
								$resulta = $connectionxx->query("SHOW TABLE STATUS FROM $database LIKE 'recaudo_rcgt'");
								while ($array = $resulta->fetch_assoc()) {
									$consec_recaudo2 = $array['Auto_increment'];
								}

								?>
								<input name="consec_ncbt" type="hidden" id="consec_ncbt" value="<?php printf("%s", $consec_recaudo2); ?>" />
												...
												<div style="font-weight: bold; text-align: center; color: #FFFFFF; padding: 5px; background-color:#006394">
													<table border="0" width="100%">
														<tr>
															...
															<td align="right"><img src="../simbolos/cerrar.png" width="15" border="0" onclick="ocultarVentana();" onmouseover="Puntero();" onmouseout="PunteroNormal();" /></td>
														</tr>
													</table>
												</div>
												<iframe id="datamain" src="rcgtconsecutivo.php" width="200" height="290" marginwidth="0" marginheight="1" hspace="0" vspace="0" frameborder="0" scrolling="si"> </iframe>
											</div>
											<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
						...
						<table width="800" border="0" align="center" class="bordepunteado1">
							...
							<tr bgcolor="#DCE9E5">
								<td class="Titulofor" align="left" colspan="2">
									<div style="padding-left:30px;">Rubro:</div>
								</td>
								<td class="Titulofor" align="center">
									<div style="padding-left:150px;">Valor:</div>
								</td>
								<td class="Titulofor" align="center">
									<div style="padding-left:15px;">Descuentos:</div>
								</td>
							</tr>

							<tr>
								<td colspan="4" bgcolor="#FFFFFF">
									<div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:15px;">
                            ...
                            <td align='left' width='20%'><input name='valor_$k' type='text' class='Estilo4' id='valor_$k' size='20' onkeypress='return validar(event)' onFocus='tipo_cuenta(id);' style='text-align:right' onblur='formato(id,value)' /></td>
								...
								<td colspan="4">
									<div class="Estilo12" style="padding-left:5px; padding-top:10px; padding-right:5px; padding-bottom:10px;">
										<div align="center">
											...
											<span class="Estilo8">:::</span>
											<input name="submit" type="submit" id="btn2" class="Estilo12" value="Guardar Recibo de caja" onclick="return validaForm()" />
										</div>
									</div>
								</td>
							</tr>
							...
							<div align="center">
								...
								<div style='padding-left:3px; padding-top:3px; padding-right:3px; padding-bottom:3px; background:#004080; width:150px'>
									<div style='padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:5px; background:#FFFFFF'>
										<div align="center"><a href='../recaudos_tesoreria/recaudos_tesoreria.php?a=RCGT&nn=RCGT' target='_parent'>VOLVER </a> </div>
									</div>
								</div>
							</div>
						</div>
