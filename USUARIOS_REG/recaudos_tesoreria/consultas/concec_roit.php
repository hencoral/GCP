...
$cx = new mysqli($server, $dbuser, $dbpass, $database) or die ("Fallo en la Conexion a la Base de Datos");
		...
		if ($concec)
		{
			...
			do {
					$con = $concec + $i;
					$con2 ="ROIT".$con;
					$sq2 =mysql_db_query($database,"select * from recaudo_roit where id_manu_roit ='$con2'",$cx);
					$fil = mysql_num_rows($sq2);
					$conant = $con-1;
					$conant2 = "ROIT".$conant;
					$sq3 =mysql_db_query($database,"select * from recaudo_roit where id_manu_roit ='$conant2'",$cx);
					$row3 = mysql_fetch_array($sq3);
					$fecha2 =$row3["id_manu_roit"]; 
					if ($fil >0)
					{
						$i++;
						$j =0;
