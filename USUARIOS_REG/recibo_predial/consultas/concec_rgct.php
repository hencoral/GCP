...
$cx = new mysqli($server, $dbuser, $dbpass, $database) or die ("Fallo en la Conexion a la Base de Datos");
		...
		$sql2 = "select max(id_manu_rcgt) from recaudo_riip where fecha_recaudo='$fecha'";
		...
		if ($concec)
		{
			...
			do {
					$con = $concec + $i;
					$con2 ="RIIP".$con;
					$sq2 =mysql_db_query($database,"select * from recaudo_riip where id_manu_rcgt ='$con2'",$cx);
					$fil = mysql_num_rows($sq2);
					$conant = $con-1;
					$conant2 = "RIIP".$conant;
					$sq3 =mysql_db_query($database,"select * from recaudo_riip where id_manu_rcgt ='$conant2'",$cx);
		...
		// si la fecha no tiene registros
			...
			do {
					...
					$sql4 = "select max(id_manu_rcgt) from recaudo_riip where fecha_recaudo='$fecha_b'";
