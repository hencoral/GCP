...
$sq3="select * from farm_lab order by lab asc";
$re3=mysql_query($sq3,$cx);
while($rw3 = mysql_fetch_array($re3))
{
	$link ="onclick=cargaArchivo(&#34admin/inicial/lab_form.php?tipo_art=$rw3[cod_art]&id=$rw3[id]&nombre=$rw3[lab]&#34,&#34form33&#34)";	
	//colorear filas
	$cont++;
		...
		<tr id='$rw3[id]' onmouseover=cambiaColor(id); onmouseout=fueraPuntero(id);>
			...
			<td valign='middle' align='center'>
				<div id='acciones$rw3[id]' style='display:none' class='actions'>
					...
					<a href='#' onclick=borrarRegistro('admin/inicial/lab_eliminar.php?id=$rw3[id]','reporte')><i class='fa fa-trash-o'></i></a>
				</div>
			</td>
		</tr>");
		$total = $total + $subtot;
