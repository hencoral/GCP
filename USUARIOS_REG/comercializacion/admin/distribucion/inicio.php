...
function buscarBarras(){
	...
	ajax=objetoAjax();
	//uso del medotod GET
	ajax.open("POST","admin/distribucion/consultas/codigo.php",false);
...
function buscarProducto(valor){
	...
	ajax=objetoAjax();
	//uso del medotod GET
	ajax.open("POST","admin/distribucion/consultas/saldo.php",false);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			//mostrar resultados en esta capa
			var res = ajax.responseText
...
function buscaFactura(){
	...
	ajax=objetoAjax();
	//uso del medotod GET
	ajax.open("POST","admin/distribucion/consultas/factura.php",false);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			//mostrar resultados en esta capa
			var res = ajax.responseText
...
function llamaMenu2()
{
	...
	var archivo = 'admin/distribucion/reporte.php?factura='+id;
...
function nuevoArt()
{
	...
	if (error ==0)
	{
	var archivo = 'admin/distribucion/agregarpro.php?bodega='+bodega+'&tipo_art='+tipo_art+'&fecha='+fecha+'&concepto='+concepto+'&acta='+acta+'&codigo='+codigo+'&barras='+barras+'&lote='+lote+'&fecha_ven='+fecha_ven+'&cant='+cant+'&cc='+cc+'&venta='+venta+'&Invima='+Invima+'&id='+id;
	cargaArchivo(archivo,'reporte');
	document.getElementById('codigo').value='';
	document.getElementById('barras').value='';
	document.getElementById('lote').value='';
...
<table width="90%" border="0" class="punteado" cellpadding='3' cellspacing='0' align="center"> 
	<tr>
    	<td colspan="4" align="left" class="menu3"> DISTRIBUCION DE MEDICAMENTOS E INSUMOS MEDICOS</td>
   ...
   <table width="90%" border="0" class="punteado" cellpadding='0' cellspacing='0' align="center">  
     ...
     <tr>
         ...
         <td><select name="cc" id="cc" >
						...
						?>
 			</select><label id="cc_e" style="color:#F00"></label>
                </td>
     </tr>
     <tr>
         <td>Concepto:</td>
         <td><input type='text' name='concepto' id='concepto' size='60' value='DISTRIBUCION DE MEDICAMENTO O INSUMOS MEDICOS' /> 
               </td>
     </tr>
     <tr>  
         <td>No factura:</td>
