...
function buscarBarras(){
	...
	ajax=objetoAjax();
	//uso del medotod GET
	ajax.open("POST","admin/recepcion/consultas/codigo.php",false);
...
function buscarProducto(valor){
	...
	ajax=objetoAjax();
	//uso del medotod GET
	ajax.open("POST","admin/recepcion/consultas/saldo.php",false);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			//mostrar resultados en esta capa
			var res = ajax.responseText
			...
			document.getElementById('venta').value=formatea(result[3]);
			document.getElementById('Invima').value=result[2];
			document.getElementById('barras').value=result[0];
			document.getElementById('compra').value=formatea(result[4]);
			//mostrar el formulario
			// divFormulario.style.display="block";
			document.getElementById('menu1_fecha_ven').value='';
			document.getElementById('mes').value='';
			document.getElementById('Invima').focus();
		}
	}
	//como hacemos uso del metodo GET
	//colocamos null
...
function buscaFactura(){
	...
	ajax=objetoAjax();
	//uso del medotod GET
	ajax.open("POST","admin/recepcion/consultas/factura.php",false);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			//mostrar resultados en esta capa
			var res = ajax.responseText
...
function nuevoArt()
{
	...
	var compra = escape(document.getElementById('compra').value);
	var Invima = escape(document.getElementById('Invima').value);
	// campos para validar
	var camposval = ["bodega", "tipo_art", "acta","codigo","cant","valtercero","venta","compra"]; // 6
	var cuantos =8;// depende del numero de campos a validar
	var i =0;
	for (i=0;i<cuantos;i++)
	{	
	...
	if (error ==0)
	{
	var archivo = 'admin/recepcion/agregarpro.php?bodega='+bodega+'&tipo_art='+tipo_art+'&fecha='+fecha+'&concepto='+concepto+'&acta='+acta+'&codigo='+codigo+'&barras='+barras+'&lote='+lote+'&fecha_ven='+fecha_ven+'&cant='+cant+'&valtercero='+valtercero+'&venta='+venta+'&Invima='+Invima+'&id='+id+'&compra='+compra;
	cargaArchivo(archivo,'reporte');
	document.getElementById('codigo').value='';
	document.getElementById('barras').value='';
	document.getElementById('lote').value='';
...
function Ventana()
{
	...
	var tipo_art = escape(document.getElementById('tipo_art').value);
	archivo ='admin/recepcion/formulario.php?tipo_art='+tipo_art;
...
function parametros()
{
	...
	document.getElementById('producto').value='';
	document.getElementById('codigo').value='';
	var archivo = 'admin/recepcion/guarda.php?bodega='+bodega+'&articulo='+articulo;
...
// ****** de este post **********
function verMes(valor)
{
	// defino los meses con sus terminales en un array
	var dias = ["","31", "28", "31","30","31","30","31","31", "30", "31","30","31"]; // 
	var d = new Date();
	var anno = parseInt(d.getFullYear()+2);
	var caract = valor.length;
	var mes = valor;
	if (caract == 1) mes = 0+valor;
	document.getElementById('menu1_fecha_ven').value=anno+"/"+mes+"/"+dias[valor];
	if (valor =='')
	document.getElementById('menu1_fecha_ven').value='';
}
</script>
...
<table width="90%" border="0" class="punteado" cellpadding='3' cellspacing='0' align="center"> 
	<tr>
    	<td colspan="4" align="left" class="menu3">RECEPCION DE MEDICAMENTO E INSUMOS MEDICOS</td>
 ...
 <table width="90%" border="0" class="punteado" cellpadding='0' cellspacing='0' align="center">   
   ...
   <table width="90%" border="0" class="punteado" cellpadding='0' cellspacing='0' align="center">  
     ...
     <tr>  
         <td>Tipo Articulo:</td>
         <td id="articulo_llega"><select name="tipo_art" id="tipo_art" onchange="parametros();" >
                        <option value="" ></option>
						<?php
						...
						?>
                        </select><label id="tipo_art_e" style="color:#F00"></label></td>
     </tr>
   	 <tr>  
         <td>Fecha:</td>
     ...
     <tr>
         <td>Concepto:</td>
         <td><input type='text' name='concepto' id='concepto' size='60' value='RECEPCION DE MEDICAMENTO O INSUMOS MEDICOS' /> 
               </td>
     </tr>
     <tr>  
         <td>No factura:</td>
...
<script>
$().ready(function() {
	$("#tercero").autocomplete("admin/recepcion/consultas/terceros.php", {
		width: 500,
		matchContains: true,
		mustMatch: true,
		//minChars: 0,
