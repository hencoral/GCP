...
$sq3="select * from farm_kardex where doc_num ='$factura' and doc_ref ='3'";
$re3=mysql_query($sq3,$cx);
while($rw3 = mysql_fetch_array($re3))
{
	$link ="onclick=cargaArchivo(&#34admin/recepcion/producto3.php?cod=$rw3[cod_int]&id=$rw3[id]&#34,&#34producto2&#34)";	
	//colorear filas
	$cont++;
	...
	// Consuto las cantidades que han entrado
		...
		<tr id='$rw3[id]' onmouseover=cambiaColor(id); onmouseout=fueraPuntero(id);>
			<td  $link align='left'>$rw3[entrada]</td>
			...
			<td valign='middle' align='center'>
				<div id='acciones$rw3[id]' style='display:none' class='actions'>
					...
					<a href='#' onclick=borrarRegistro('admin/recepcion/eliminar.php?id=$rw3[id]&factura=$factura','reporte')><i class='fa fa-trash-o'></i></a>
				</div>
			</td>
		</tr>");
		$total = $total + $subtot;
