...
function buscarLote(){
	...
	ajax=objetoAjax();
	//uso del medotod GET
	ajax.open("POST","admin/administra/consultas/saldo_lote.php",false);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			//mostrar resultados en esta capa
			var res = ajax.responseText
	...
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	//enviando los valores
	ajax.send("codigo="+codigo+"&tipo="+tipo_art+"&lote="+lote);
}
function buscaFactura(){
	...
	ajax=objetoAjax();
	//uso del medotod GET
	ajax.open("POST","admin/administra/consultas/factura.php",false);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			//mostrar resultados en esta capa
			var res = ajax.responseText
...
function llamaMenu2()
{
	...
	var archivo = 'admin/administra/reporte.php?factura='+id;
...
function nuevoArt()
{
	...
	// campos para validar
	var camposval = ["bodega", "tipo_art", "acta","codigo","cant","concepto","venta"]; // 6
	...
	if (error ==0)
	{
	var archivo = 'admin/administra/agregarpro.php?bodega='+bodega+'&tipo_art='+tipo_art+'&fecha='+fecha+'&concepto='+concepto+'&acta='+acta+'&codigo='+codigo+'&barras='+barras+'&lote='+lote+'&fecha_ven='+fecha_ven+'&cant='+cant+'&venta='+venta+'&Invima='+Invima+'&id='+id;
	cargaArchivo(archivo,'reporte');
	document.getElementById('codigo').value='';
	document.getElementById('barras').value='';
	document.getElementById('lote').value='';
...
<table width="90%" border="0" class="punteado" cellpadding='3' cellspacing='0' align="center"> 
	<tr>
    	<td colspan="4" align="left" class="menu3">ADMINISTRACION DE MEDICAMENTO E INSUMOS MEDICOS</td>
 ...
 <table width="90%" border="0" class="punteado" cellpadding='0' cellspacing='0' align="center">   
   ...
   <table width="90%" border="0" class="punteado" cellpadding='0' cellspacing='0' align="center">  
         ...
         <td> <select name="concepto" id="concepto">
                        ...
                        for ($i=0; $i<$fi2; $i++)
						{
                            $rw2 = mysql_fetch_array($rs2);
		                            ...
		                            echo "<OPTION VALUE='$rw2[prefijo]'>$rw2[admin]</OPTION>";
        				}
						?>
                 </select>       
                 <label id="concepto_e" style="color:#F00"></label>
                </td>
     </tr>
     ...
     <tr>  
         ...
         &nbsp;<i class="fa fa-plus-square" style="font-size:20px;color:#06F;cursor:pointer" aria-hidden="true" title="Nueva acta" onclick="buscaFactura();"></i>
