...
$sql2 = "select max(id_manu_rcgt) from recaudo_rcgt where fecha_recaudo='$fecha' and id_manu_rcgt like 'RCGT%'";
$res = $cx->query($sql2);
$row = $res->fetch_assoc();
...
if ($concec) {
	...
	do {
		$con = $concec + $i;
		$con2 = "RCGT" . $con;
		$sq2 = $cx->query("select * from recaudo_rcgt where id_manu_rcgt ='$con2'");
		$fil = $sq2->num_rows;
		$conant = $con - 1;
		$conant2 = "RCGT" . $conant;
		$sq3 = $cx->query("select * from recaudo_rcgt where id_manu_rcgt ='$conant2'");
	...
	do {
		...
		$sql4 = "select max(id_manu_rcgt) from recaudo_rcgt where fecha_recaudo='$fecha_b' and id_manu_rcgt like 'RCGT%'";
		$res4 = $cx->query($sql4);
