...
?>
	...
	<body>
		...
		<table width="800" border="0" align="center">
			...
			<tr>
				<td align="center" colspan="10">
					<font size="4"><b>REPORTE MODIFICACIONES MENSUALES PRESUPUESTO DE GASTOS</b></font>
				</td>
			</tr>
			<tr>
				<td align="center" colspan="10">
...
");
		...
		while ($rw = $re->fetch_assoc()) {
			...
			$resulta = $link->query("SELECT SUM(valor_adi) AS TOTAL from adi_ppto_gas WHERE (fecha_adi between '$fecha_ini' and '$fecha_fin' ) and id_emp='$id_emp' and cod_pptal LIKE '$cod%'");
			$row = $resulta->fetch_array();
			$total_adi = $row[0];

			$resulta2 = $link->query("SELECT SUM(valor_adi) AS TOTAL from red_ppto_gas WHERE (fecha_adi between '$fecha_ini' and '$fecha_fin' ) and id_emp='$id_emp' and cod_pptal LIKE '$cod%'");
			$row2 = $resulta2->fetch_array();
			$total_red = $row2[0];

			$resulta3 = $link->query("SELECT SUM(valor_adi) AS TOTAL from creditos WHERE (fecha_adi between '$fecha_ini' and '$fecha_fin' ) and id_emp='$id_emp' and cod_pptal LIKE '$cod%'");
			$row3 = $resulta3->fetch_array();
			$total_cre = $row3[0];

			$resulta4 = $link->query("SELECT SUM(valor_adi) AS TOTAL from contracreditos WHERE (fecha_adi between '$fecha_ini' and '$fecha_fin' ) and id_emp='$id_emp' and cod_pptal LIKE '$cod%'");
			$row4 = $resulta4->fetch_array();
			$total_ccre = $row4[0];

			$resulta = $link->query("SELECT SUM(valor_aplazado) AS TOTAL from aplazamientos WHERE (fecha_adi between '$fecha_ini' and '$fecha_fin' ) and cod_pptal LIKE '$cod%'");
			$row = $resulta->fetch_array();
			$total5 = $row[0];
			$total_apla = $total5;

			$resulta = $link->query("SELECT SUM( valor_levantado) AS TOTAL from levanta_aplazamientos WHERE (fecha_adi between '$fecha_ini' and '$fecha_fin' ) and cod_pptal LIKE '$cod%'");
			$row = $resulta->fetch_array();
			$total5 = $row[0];
...
</tr>", $rw["cod_pptal"], utf8_decode($rw["nom_rubro"]), number_format($total_adi, 2, ',', '.'), number_format($total_red, 2, ',', '.'), number_format($total_cre, 2, ',', '.'), number_format($total_ccre, 2, ',', '.'),  number_format($total_apla, 2, ',', '.'), number_format($total_desapla, 2, ',', '.'), $rw["tip_dato"], $rw["nivel"]);
			}
		}
