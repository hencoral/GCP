...
function Calcular(){
  ...
  for(var i = 1;i <= can; i++){
    ...
    if(document.getElementById(co1).value != ""){
        to1 = to1 + parseFloat(document.getElementById(co1).value.replace(/\,/g,''));}
    if(document.getElementById(co2).value != ""){
        to2 = to2 + parseFloat(document.getElementById(co2).value.replace(/\,/g,''));}
  }
  document.getElementById("tot_deb_a").value = Math.round(to1*100)/100;
  document.getElementById("tot_cre_a").value = Math.round(to2*100)/100;
  dif = to1-to2;
  document.getElementById("total").value = Math.round(dif*100)/100;
}
...
function noVacio4(Form,val,des,has){
 var freg=document.getElementById("fecha_rec").value;
  var fca=document.getElementById("fecha_ca").value;
 var fven=document.getElementById("fecha_ven").value;

if(fca<freg)
{
	 alert("La fecha de Causacion no puede ser menor a la fecha de Reconocimiento...");
	 document.getElementById("fecha_ca").focus();
	 return false;
 }

 if(fven<fca)
 {
	 alert("La fecha de vencimiento no puede ser menor a la fecha de Causacion...");
	 document.getElementById("fecha_ven").focus();
	 return false;
 }
