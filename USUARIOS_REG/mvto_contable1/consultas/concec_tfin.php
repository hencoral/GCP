...
// Realizo la edicion de los datos
		$sq2 ="select max(id_manu_ncon) as dcto from conta_tfin where id_manu_ncon like 'TFIN%' and  fecha_ncon between '$anno2' and '$fecha' ";
		$rs2 = mysql_query($sq2);
		...
		if ($concec !='')
		{
			do{
				...
				$sq3 ="select id_manu_ncon from conta_tfin where id_manu_ncon = '$con'";
				$re3 = mysql_query($sq3);
				...
				if($fil ==0)
				{
					...
					$id = "TFIN".($concec-1);
					$sq4 ="select fecha_ncon from conta_tfin where id_manu_ncon = '$id'";
					$re4 = mysql_query($sq4);
