...
$cx = new mysqli($server, $dbuser, $dbpass, $database) or die ("Fallo en la Conexion a la Base de Datos");
		...
		$sql2 = "select max(id_manu_ncon) from conta_ncon where fecha_ncon='$fecha'";
		$res = mysql_db_query($database,$sql2,$cx);
		$row = mysql_fetch_array($res);
		$dato = $row["max(id_manu_ncon)"];
		...
		if ($concec)
		{
			...
			do {
					$con = $concec + $i;
					$con2 ="NCON".$con;
					$sq2 =mysql_db_query($database,"select * from conta_ncon where id_manu_ncon ='$con2'",$cx);
					$fil = mysql_num_rows($sq2);
					$conant = $con-1;
					$conant2 = "NCON".$conant;
					$sq3 =mysql_db_query($database,"select * from conta_ncon where id_manu_ncon ='$conant2'",$cx);
					$row3 = mysql_fetch_array($sq3);
					$fecha2 =$row3["fecha_ncon"]; 
					if ($fil >0)
					{
						$i++;
						$j =0;
