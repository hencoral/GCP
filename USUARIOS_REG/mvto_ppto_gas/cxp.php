...
<body>
	...
	while ($row2 = $re2->fetch_assoc()) {
		...
		if ($filas == 0) {
			...
			$rw5 = $rs5->fetch_assoc();
			// para sacar 
			$sq6 = "select n_contrato from crpp where id_auto_crpp ='$row2[id_auto_crpp]'";
			$rs6 = $cx->query($sq6);
			$rw6 = $rs6->fetch_assoc();
			// Por edades
			if ($edad <= 60) $sesenta = $rw4['valors'];
			else $sesenta = 0;
			if ($edad > 60 and $edad <= 90) $noventa = $rw4['valors'];
			else $noventa = 0;
			if ($edad > 90 and $edad <= 180) $cochenta = $rw4['valors'];
			else $cochenta = 0;
			if ($edad > 180 and $edad <= 360) $tresesenta = $rw4['valors'];
			else $tresesenta = 0;
			if ($edad > 360) $cresesenta = $rw4['valors'];
			else $cresesenta = 0;
