...
header("Content-Disposition: attachment; filename=Fut_Ingresos_Reporte.xls");
...
$cott= 1;
	...
	<td align='center'>REPORTE_INFORMACION</td>
	<td align='center'></td>
	<td align='center'></td>
	<td align='center'></td>
	<td align='center'></td>
	...
	{ 
			...
			",'D',$rw["cod_fut"],$rw["sum(ppto_aprob)"],$rw["sum(definitivo)"], $rw["sum(vr_efectivo)"],$rw["sum(vr_sin_sit)"],$rw["sum(vr_efectivo)"] + $rw["sum(vr_sin_sit)"], $rw["acto_fut"],$rw["num_acto_fut"],$rw["porcentaje_fut"],$rw["sum(vr_fut)"]); 
		}  
	}
	}
	$rec = $rec_cf + $rec_sf;
printf("
	<tr>
		...
		<td align='right'>$defin</td>
		<td align='right'>$rec_cf</td>
		<td align='right'>$rec_sf</td>
