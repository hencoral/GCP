...
if ($mes == 1) {$fecha_ini =$anno."/01/01"; $fecha_fin = $anno."/03/31";}
if ($mes == 2) {$fecha_ini =$anno."/04/01"; $fecha_fin = $anno."/06/30";}
if ($mes == 3) {$fecha_ini =$anno."/07/01"; $fecha_fin = $anno."/09/30";}
if ($mes == 4) {$fecha_ini =$anno."/10/01"; $fecha_fin = $anno."/12/31";}
if ($mes == 5) {$fecha_ini =$anno."/01/01"; $fecha_fin = $anno."/12/31";}

if ($mes ==5) $ver = ""; else $ver ="style='display:none'"; 

$mes_lit =array('','ENERO - MARZO','ABRIL - JUNIO','JULIO - SEPTIEMBRE','OCTUBRE - DICIEMBRE','ANUAL');
	$i=1;
	for ($i=1;$i<=5;$i++)
	{
