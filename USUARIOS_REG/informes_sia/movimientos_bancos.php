...
for ($i=0;$i<12;$i++)
{
	...
	// consultar el saldo en libros segun la fecha de corte
		...
		if ($n_cred =='') $n_cred=0;
		//Sumo notas debito
		$sq6 = "select SUM(credito) as ndebito from lib_aux where cuenta = '$cuenta' and (fecha BETWEEN '$fini_mes' and '$ffin_mes') and (dcto like 'NDSP%' or dcto like 'NCON%' OR dcto LIKE 'TFIN%' OR dcto LIKE 'COBA%')";
	...
	// filtra periodo para el informe
	if ($mes2 == $i)
		{
	printf ("	<tr>
