...
function chk_rcgt(){
var pos_url = '../comprobadores/comprueba_rica.php';
...
function agregar()
 {
	...
	td = tr.insertCell();
	td.innerHTML = "<div style='padding-left:5px; padding-top:3px; padding-right:5px; padding-bottom:3px;'><input type='text' size='13' style='text-align:left' id='pgcp"+contLin+"' name='pgcp"+contLin+"' value='' onkeyup='lookup(this.value,"+contLin+");' onkeydown='displaycode(event,id);' alt='2'></div> <div class='suggestionsBox' id='sugges"+contLin+"' style='display: none; position:absolute; left: 130px;'><img src='../simbolos/upArrow.png' style='position: relative; top: -10px; left: 0px;' title='PGCP'> <div class='suggestionList' id='autoSug"+contLin+"' align=left> </div> </div>";
	
	td = tr.insertCell();
	td.innerHTML = "<input type='text' size='35' style='text-align:left' name='des"+contLin+"' id='des"+contLin+"' value='' readonly >";
	
	td = tr.insertCell();
	td.innerHTML = "<input type='text' size='15' style='text-align:right' name='vr_deb_"+contLin+"' id='vr_deb_" + contLin + "' value=0   onKeyUp='suma_tab();' onkeypress='return validar(event)' onkeydown='displaycode(event,id);' onblur='formato(id,value);' onfocus='siespgcp("+contLin+");' alt='2' >";
	
	td = tr.insertCell();
	td.innerHTML = "<input type='text' size='15' style='text-align:right' name='vr_cre_" + contLin + "' id='vr_cre_" + contLin + "' value=0  onKeyUp='suma_tab();' onkeypress='return validar(event)'   onkeydown='displaycode(event,id);' onblur='formato(id,value);' alt='2'>";
	td = tr.insertCell();
			...
			{																	
				req1.onreadystatechange = function() 
				{
					if (req1.readyState == 4 ) 
					{
						...
						if (j==14)
						{
							var otro = parseFloat(document.getElementById("conta_14").value.replace(/\./g,''));
							if (otro >0 && otro < 10000)
							{
								document.getElementById("pgcp"+k).value='140190';
								document.getElementById("des"+k).value='Otros deudores por ingresos no tributarios';
								document.getElementById("vr_cre_"+k).value=document.getElementById("conta_14").value;
							}
							if (otro > 9999)
							{
								document.getElementById("pgcp"+k).value='24251201';
								document.getElementById("des"+k).value='Saldos a favor industria y comercio';
								document.getElementById("vr_cre_"+k).value=document.getElementById("conta_14").value;
							}
						}
...
var pred_ant =0;
var pred_act =0;
var tot_conta =parseInt(document.getElementById('conta_14').value.replace(/\./g,''));		
 for (i=1;i<=10;i++)
    {
...
function procesar_reporte()
{
	...
	var otros =  parseInt(document.getElementById('conta_14').value.replace(/\./g,''));
	var total = avisos+ley;
	for(j=0;j<filas;j++)
	{
		menos_inputs();	
	}
	...
	// Cargo presupuesto otros cuando el valor de otros es menor a 10.000 y mayor a cero
	
	if (otros >0 && otros < 10000)
	{
		k=k+1;
		mas_inputs();	
		document.getElementById('course_'+k).value='11010207  - Otros ingresos no tributarios';
		document.getElementById('valcourse_'+k).value='11010207';
		document.getElementById('valor_'+k).value=otros;
...
function sum_press()
{
	...
	for (k=1;k<=filas;k++)
	{
		valor =parseInt(document.getElementById('valor_'+k).value.replace(/\./g,''));
		suma = suma + valor;
		document.getElementById('totals').value= formatea(suma);
	}	
}
...
function compensa(cod)
{
var dato='';
var pos_url2 = 'consultas/compensa_ica.php';	
var req1 = new XMLHttpRequest();	
	if (req1)
	{																	
		req1.onreadystatechange = function() 
	...
	req1.open('POST', pos_url2 +'?cod='+cod,false);
	req1.send(null);
	}
	 return dato;	
}

function bcompen(codigo)
{
	var datos =compensa(codigo);
	if (datos)
	{
		...
		for (i=3;i<=12;i++)
		{
			document.getElementById('compe_'+i).value=formatea(parseInt(element[i-2]));	
		}
	}
}
var valors = new Array(12);
function guarda_valores ()
{
	var j=0;var suma=0;
	for	(j=1;j<=13;j++)
	{
		valors[j]=document.getElementById('conta_'+j).value;
	}
...
function aplicar()
{
	var j=0;var suma=0;
		for	(j=1;j<=12;j++)
	{
		document.getElementById('conta_'+j).value=	formatea(document.getElementById('conta_'+j).value.replace(/\./g,'') - document.getElementById('compe_'+j).value.replace(/\./g,''));
	suma = suma + parseInt(document.getElementById('compe_'+j).value.replace(/\./g,''));
	}
	document.getElementById('conta_11').value= parseInt(document.getElementById('conta_11').value.replace(/\./g,'') - suma);
}
...
function deshacer()
{
		...
		for (k=1;k<=12;k++)
		{
			document.getElementById('conta_'+k).value=valors[k];	
		}
}
...
<body onload="consecutivo2();">
 <form name="a" method="post" id="commentForm" action="proc_rcgt.php">
...
$resulta = mysql_query("SHOW TABLE STATUS FROM $database LIKE 'recaudo_rica'");
      ...
      <tr>
        ...
        <td><div style="padding-left:5px; padding-top:10px; padding-right:5px; padding-bottom:10px;">
          <div align="center"><strong class="Estilo4">Consecutivo:</strong>
            <input name="id_manu_rcgt" type="text" class="required Estilo12" id="id_manu_rcgt" size="20" onkeypress="return validar(event)" style="text-align:center" onkeyup="chk_rcgt();" />
      ...
      <tr>
        ...
        <td colspan="3"><div style="padding-left:10px; padding-top:5px; padding-right:5px; padding-bottom:5px;">
            <div align="center" class="Estilo4">
                  ...
                  <div align="left">
                    <input name="des_recaudo" type="text" class="required Estilo4" id="des_recaudo"  size="80" alt="1"  />
                    </div>
            </div></div></td>
      </tr>
    ...
    <table width="850" border="0" align="center" class="bordepunteado1">
      ...
      <tr>
        <td colspan="4" bgcolor="#FFFFFF"><div style="padding-left:5px; padding-top:5px; padding-right:5px; padding-bottom:15px;">
            ...
            <table width="60%" border="0" align="center" id="tablei">
            	<tr class="subtitle">
                	<td>Concepto</td>
                    <td align="center">Valor</td>
                    <td align="center">Compensaci�n</td>
                </tr>
                <tr>
                	<td>TIPO</td>
                    <td><select name="tipo" id="tipo" style='background:#F4F4EA;'>
                    		<option value=" "> </option>
                            <option value="VAL">VIG. ACTUAL</option>
                            <option value="VAN">VIG. ANTERIOR</option>
                            <option value="INS">INSCRIPCION</option>
                        </select>
                    </td>
                    <td>  </td>
				</tr>
               ...
               <tr>
                    ...
                    <td><input name="ica" id="ica" size="18" style='text-align:left; background:#F4F4EA' value="" onfocus="vaciar(id);" alt="1" onblur="bcompen(value);"/></td>
                    <td align="center"><input type="button" name="btn5"  value="&nbsp;&nbsp;Aplicar&nbsp;&nbsp;" style="background:#72A0CF; color:#FFFFFF; border:none" onclick="aplicar();" /><input type="button" name="btn6"  value="<<" style="background:#9F3; color:#FFFFFF; border:none" onclick="deshacer()" /></td>
				</tr>

                <tr>
                	<td>Inscripcion</td>
                    <td><input name="conta_1" id="conta_1" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_1" id="compe_1" size="18" style='text-align:right' value="0"  /></td>
                </tr>
               	<tr>
                	<td>Patente</td>
                    <td><input name="conta_2" id="conta_2" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_2" id="compe_2" size="18" style='text-align:right' value="0"  /></td>
                </tr>
            	<tr>
                	<td>Impuesto Ley 14</td>
                    <td><input name="conta_3" id="conta_3" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_3" id="compe_3" size="18" style='text-align:right' value="0"  /></td>
                </tr>
               	<tr>
                	<td>avisos y tableros</td>
                    <td><input name="conta_4" id="conta_4" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_4" id="compe_4" size="18" style='text-align:right' value="0"/></td>
                </tr>
            	<tr>
                	<td>Sobretasa bomberil</td>
                    <td><input name="conta_5" id="conta_5" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_5" id="compe_5" size="18" style='text-align:right' value="0"/></td>
                </tr>
               	<tr>
                	<td>Estampilla procultura</td>
                    <td><input name="conta_6" id="conta_6" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_6" id="compe_6" size="18" style='text-align:right' value="0"/></td>
                </tr>
            	<tr>
                	<td>Certificado industria y comercio</td>
                    <td><input name="conta_7" id="conta_7" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_7" id="compe_7" size="18" style='text-align:right' value="0"/></td>
                </tr>
               	<tr>
                	<td>Extemporaneidad</td>
                    <td><input name="conta_8" id="conta_8" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_8" id="compe_8" size="18" style='text-align:right'  value="0"/></td>
                </tr>
            	<tr>
                	<td>Otras sanciones</td>
                    <td><input name="conta_9" id="conta_9" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_9" id="compe_9" size="18" style='text-align:right' value="0" /></td>
                </tr>
            	<tr>
                	<td>Intereses</td>
                    <td><input name="conta_10" id="conta_10" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_10" id="compe_10" size="18" style='text-align:right' value="0"/></td>
                </tr>
            	<tr>
                	<td>Saldo a favor</td>
                    <td><input name="conta_11" id="conta_11" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td><td><input name="compe_11" id="compe_11" size="18" style='text-align:right' value="0" /></td>
                </tr>
            	<tr>
                	<td>Descuentos Imp</td>
                    <td><input name="conta_12" id="conta_12" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_12" id="compe_12" size="18" style='text-align:right' value="0" /></td>
                </tr>
                <tr>
                	<td>Descuentos Avisos</td>
                    <td><input name="conta_13" id="conta_13" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_13" id="compe_13" size="18" style='text-align:right' value="0"/></td>
                </tr>
                <tr>
                	<td>Otros ingresos</td>
                    <td><input name="conta_14" id="conta_14" size="18" style='text-align:right' onblur="sumac(id);" onkeyup="totales();" value="0" onfocus="vaciar(id);" alt="1" /></td>
                    <td><input name="compe_14" id="compe_14" size="18" style='text-align:right' value="0"/></td>
                </tr>
               	...
               	<tr>
                    ...
                    <td><input name="compe_tot" id="compe_tot" size="18" style='text-align:right' disabled="disabled"/></td>
                </tr>
                <tr>
               	<td colspan="2" height="15" align="center"> </td><td>&nbsp;</td>
                </tr>
            ...
            <table width="95%" border="0" align="center" id="tablei">
                    ...
                    for ($k=1;$k<=20;$k++)
					{
                      echo "<tr style='display:$ver;' id='inputx$k'>
                            ...
                            <td align='left' width='20%'><input name='valor_$k' type='text' class='Estilo4' id='valor_$k' size='20' onkeypress='return validar(event)' onFocus='tipo_cuenta(id);' onblur='sumap(id);' style='text-align:right' /></td>
					...
					echo "<tr id='inputx$k'>
                            ...
                            <td align='left'><input type='text' name='totals' id='totals' value='0' style='text-align:right; border:0;'  /></td>
							<td align='left'></td>
                           </tr>";
					?>
